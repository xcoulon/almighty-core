...
func (s *JSONComplianceTestSuite) NewSecuredSpaceController(identity account.Identity) (*goa.Service, *SpaceController) {
	...
	return svc, NewSpaceController(svc, s.AppDB, s.Configuration, &DummyResourceManager{})
}

func (s *JSONComplianceTestSuite) TestListSpaces() {
	// given
	fxt := tf.NewTestFixture(s.T(), s.DB, tf.Identities(1), tf.Spaces(2, func(fxt *tf.TestFixture, idx int) error {
	...
	// then
	jsonSpaceList, err := json.Marshal(spaceList)
	require.Nil(s.T(), err)
