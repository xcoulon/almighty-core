...
func (c *SpaceController) Create(ctx *app.CreateSpaceContext) error {
	...
	resource, err := c.resourceManager.CreateResource(ctx, ctx.Request, spaceID.String(), spaceResourceType, &spaceName, &scopes, currentUser.String())
	if err != nil {
		return jsonapi.JSONErrorResponse(ctx, err)
	}
...
func (c *SpaceController) Delete(ctx *app.DeleteSpaceContext) error {
	...
	c.resourceManager.DeleteResource(ctx, ctx.Request, auth.Resource{ResourceID: resourceID, PermissionID: permissionID, PolicyID: policyID})
	if err != nil {
		return jsonapi.JSONErrorResponse(ctx, err)
	}
	return ctx.OK([]byte{})
