...
func (s *TestAuthzSuite) SetupSuite() {
	...
	s.config = configuration.Get()
	s.authzService = authz.NewAuthzService(s.config, &db{app{resource: resource}})
}
...
func (s *TestAuthzSuite) checkPermissions(authzPayload auth.AuthorizationPayload, spaceID string) bool {
	...
	entitlementEndpoint, err := s.config.GetKeycloakEndpointEntitlement(&http.Request{Host: "api.example.org"})
	require.Nil(s.T(), err)
