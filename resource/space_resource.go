...
func (r SpaceResource) GetByID(ctx *gin.Context) {
	...
	if err != nil {
		...
		ctx.AbortWithError(http.StatusInternalServerError, errs.Wrapf(err, "error while fetching the space with id=%v", spaceID.String()))
	}
	...
	if err := jsonapi.MarshalPayload(ctx.Writer, &result); err != nil {
		ctx.AbortWithError(http.StatusInternalServerError, errs.Wrapf(err, "error while fetching the space with id=", spaceID.String()))
	}
}
